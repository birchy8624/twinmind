...
import { CustomizationProvider } from './_components/customization-context'
...
function AppShellLayoutInner({ children }: AppShellLayoutProps) {
  ...
  const accountName = profile?.accountName ?? 'Workspace'
        ...
        <aside className="hidden w-72 flex-col border-r border-white/10 bg-base-900/70 p-6 backdrop-blur lg:fixed lg:inset-y-0 lg:flex">
          <div className="mb-8">
            <div className="flex items-center gap-3">
              <WorkspaceBrandmark className="h-10 w-10 flex-shrink-0" />
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-white/40">{accountName}</p>
          ...
          <header className="sticky top-0 z-40 border-b border-white/10 bg-base-900/40 px-6 py-5 backdrop-blur">
            ...
            <div className="mt-4 flex flex-col gap-3 md:hidden">
              ...
              <div className="flex items-center justify-between text-xs text-white/50">
                ...
                <span className="font-medium text-white/70">{accountName}</span>
              </div>
            ...
            >
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <WorkspaceBrandmark className="h-9 w-9 flex-shrink-0" />
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-white/40">{accountName}</p>
                ...
                <div className="mt-6 space-y-2">{NavLinks}</div>
                <div className="mt-10 rounded-2xl border border-white/10 bg-white/5 p-5 text-sm text-white/70">
                  ...
                  <p className="mt-1 text-lg font-semibold text-white">{accountName}</p>
                  <p className="mt-2 text-xs leading-relaxed text-white/50">
                    Manage clients, projects, and the delivery pipeline from one collaborative hub designed for creative agencies.
                  </p>
                </div>
              </motion.aside>
            </motion.div>
