...
async function resolveSubscriptionMetadata(
  ...
  const currentPeriodEnd = resolveSubscriptionPeriodEnd(subscription) ?? cancelAt
...
async function applyCheckoutSession({ session }: { session: Stripe.Checkout.Session }) {
  ...
  const updatePayload: Database['public']['Tables']['subscriptions']['Update'] = {
    ...
    provider_customer_id: metadata.customerId ?? null,
  ...
  try {
    ...
    console.error('billing return post-sync error:', error)
  }
}
