...
export async function GET() {
  try {
    ...
    if (role === 'client') {
      ...
      accessibleProjectIds = (projectRows ?? [])
    ...
    const pipelinePromise = (async (): Promise<PipelineOverviewItem[]> => {
      ...
      const counts = (data ?? []).reduce((acc, project) => {
        const status = project.status as Database['public']['Enums']['project_status'] | null
    ...
    const revenuePromise = (async () => {
      let invoiceQuery = supabase.from('invoices').select('status, amount, issued_at, project_id')
      ...
      for (const invoice of data ?? []) {
        const status = invoice.status as Database['public']['Enums']['invoice_status']
    ...
    const velocityPromise = (async (): Promise<VelocityItem[]> => {
      ...
      const { data, error } = await velocityQuery
