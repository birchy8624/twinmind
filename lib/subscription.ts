...
const shouldMarkAsPaid = (status: string | null | undefined, currentPeriodEnd: string | null | undefined): boolean => {
  ...
  return isFutureDate(currentPeriodEnd)
}
...
): boolean => shouldMarkAsPaid(status, currentPeriodEnd)
...
export async function syncWorkspaceSubscription({
  ...
  const hasPaidAccess = shouldMarkAsPaid(syncedSubscription.status, syncedSubscription.current_period_end)
